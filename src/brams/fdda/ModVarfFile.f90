...
wait2:  do nw=1,nwaits
            ...
            enddo
            if (ivar_wait==0) then
                call fatal_error(h//' 1. No future varfiles found with prefix '//&
                    trim(varfpfx))
            elseif (ivar_wait==1 .and. nw<nwaits) then
                if (mchnum==master_num) then
                    print *, 'No initial varfiles found: ', trim(varfpfx)
            ...
            elseif (ivar_wait==1 .and. nw==nwaits) then
                call fatal_error(h//&
                    " Waited too long; 2. no future varfiles found with prefix "//&
                    trim(varfpfx))
            endif
    ...
    subroutine VarfFileInv(varpref, iyear1, imonth1, idate1, itime1, flag)
        ...
        if (mchnum==master_num) then
            ...
            print *, '-------------------------------------------------------------'
            do nf=1,nvarffiles
                write (*,fmt='(i4,1x,a16,1x,f10.0,2x,a)') nf, itotdate_varf(nf) &
    ...
    subroutine VarfUpdate(iswap, OneGrid, ifileok, initflag, initialFlag)
        ...
        real, pointer, dimension(:,:,:) :: varFromInit
        ...
        if(initialFlag .eq. 2)then
            ...
            if (mchnum == master_num) then
                ...
                print*,"======================================================================="
                print*,"in:",trim(h)
                write (*,fmt='(a,2x,f10.0,2x,a)') ' time (s)', time,trim(flnm(1:len_trim(flnm)))
                print*,"======================================================================="
...
!                open(unit=88,file=trim(fileName(1:len_trim(fileName)-9))//'-00.dat')
!                write(88,*) maxval(scr),minval(scr)
!                close(88)
!call writevar(scr,nnxp1(ngrid1),nnyp1(ngrid1),nnzp1(ngrid1),'mestre-'//ctime)
            endif

            call Broadcast(scr, master_num, 'scr')

           !     write(*,*) 'LFR - DEBUG:  UP',sameGrid; call flush(6)
           !     write(cmynum,fmt='(I2.2)') mynum
           !     open(unit=40+mynum,file='all-'//trim(fileName(1:len_trim(fileName)-9))//'-'//cmynum//'.dat')
           !     write(40+mynum,*) maxval(scr),minval(scr)
           !     close(40+mynum)
...
!subroutine writevar(var,xp,yp,zp,fname)
!integer, intent(in) :: xp,yp,zp
!character(len=*), intent(in) :: fname
!real, intent(in) :: var(xp,yp,zp)
!
!integer :: irec,recordLen,k
!integer, external :: outRealSize
!
!open(unit=45,file=fname//'.ctl',status='replace',action='write')
!write(45,*) 'dset ^'//fname//'.gra'
!write(45,*) 'undef  -0.9990000E+34'
!write(45,*) 'title teste UP'
!write(45,*) 'xdef',xp, 'linear',     -48.3825035,      0.0464622
!write(45,*) 'ydef',yp, 'linear',     -15.7222834,      0.0449900
!write(45,*) 'zdef',zp, 'levels',(k,k=1,zp)
!write(45,*) 'tdef 1 linear 18:00z04feb2021 1mn'
!write(45,*) 'vars  1'
!write(45,*) 'up',zp,'99 up'
!write(45,*) 'endvars'
!close(unit=45)
!
!recordLen=outRealSize()*xp*yp
!open(unit=45,file=fname//'.gra',&
!           action='WRITE',status='REPLACE',form='UNFORMATTED',access='DIRECT', &
!           recl=recordLen)
!irec=1
!do k=1,zp
!   write (45,rec=irec) var(1:xp,1:yp,k)
!   irec=irec+1
!enddo
!
!close(unit=45)
