...
contains
    ...
    subroutine radrrtmdrv(ia,iz,ja,jz,mxp,myp,mzp,mynum &
        ...
        real(kind=rb), allocatable :: hrc(:,:)           ! clear sky longwave radiative heating rate (k/d)
                                                           !        dimensions: (ncol,nlay)
        real(kind=rb), allocatable :: alpha(:,:)           !
        ...
        allocate(alpha(nofcols,nlay))         ;alpha=0.0
        ...
        call mcica_subcol_lw(iplon, nofcols, nlay, icld, changeseed, irng, play, &
            cldf, ciwp, clwp, reic, relq, taucl_lw, alpha, &
        ...
        !LFR Passando tauaer, wol e gol da RTMG para a tuv
        !Precisa checar quais as variaveis equivalentes e as dimensoes
        !Na TUV (ModTUVDrive2.7.f90) ha uma adaptacao entre as bandas da carma e TUV
        !LInhas 760 a 774 - Éh preciso verificar se está certo para iswrtyp == 6 tambem
        if(chemistry>0) then
            carma_tuv(ngrid)%g_tauaer=0.0
            carma_tuv(ngrid)%g_wol=0.0
            carma_tuv(ngrid)%g_gol=0.0
            carma_tuv(ngrid)%g_taucld=0.0
            carma_tuv(ngrid)%g_wcld=0.0
            carma_tuv(ngrid)%g_gcld=0.0
            if (trim(PhotojMethod) == 'FAST-TUV' .and. chemistry > 0)  then
                write(*,*) 'DBG-LFR: preenchendo carma_tuv...'
                noc=0
                do j=ja,jz
                    do i=ia,iz
                        noc=noc+1
                        carma_tuv(ngrid)%g_tauaer(:,:,i,j)= tauaer_sw(noc,:,:)
                        carma_tuv(ngrid)%g_wol   (:,:,i,j)= ssaaer_sw(noc,:,:)
                        carma_tuv(ngrid)%g_gol   (:,:,i,j)= asmaer_sw(noc,:,:)
                        carma_tuv(ngrid)%g_taucld(:,:,i,j)=taucmcl_sw(noc,:,:)
                        carma_tuv(ngrid)%g_wcld  (:,:,i,j)=ssacmcl_sw(noc,:,:)
                        carma_tuv(ngrid)%g_gcld  (:,:,i,j)=asmcmcl_sw(noc,:,:)
                    enddo
                end do
            endif
        endif
        ...
        do j=ja,jz
            do i=ia,iz
                ...
                if( k700 > m1 .or.   k700 < 1) then
                    ...
                    write(*,fmt='(A3,1X,3(A9,1X))') 'k','press(k)','pi0(k)','pp(k)'
                    do k=1,m1
                        write(*,fmt='(I3,1X,3(E9.3,1X))') k,press(k),basic_g(ngrid)%pi0(k,i,j),basic_g(ngrid)%pp(k,i,j)
                    enddo
