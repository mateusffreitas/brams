...
BRAMS_INSTALL_DIR=${BRAMS_INSTALL_DIR:-"${HOME}/brams-6.0-nvfortran"}
...
./configure --program-prefix=BRAMS_6.0 --prefix=${BRAMS_INSTALL_DIR} --enable-jules \
  ...
  --with-cpcomp=${PREREQ_DIR}/bin/mpicc --with-fcomp=pgf90 --with-ccomp=pgcc \
  --with-netcdff=${PREREQ_DIR} --with-netcdfc=${PREREQ_DIR} --with-wgrib2=${PREREQ_DIR}
make clean
make FPCOMP=${PREREQ_DIR}/bin/mpif90 CPCOMP=${PREREQ_DIR}/bin/mpicc CCOMP=pgcc FCOMP=pgf90 -f Make_utils
make
make install
[[ $? -eq 0 ]] || {
