...
BRAMS_INSTALL_DIR=${BRAMS_INSTALL_DIR:-"${HOME}/brams-6.0-ifort"}
...
./configure --program-prefix=BRAMS_6.0 --prefix=${BRAMS_INSTALL_DIR} --enable-jules \
  --with-chem=RELACS_TUV --with-aer=SIMPLE --with-fpcomp=${PREREQ_DIR}/bin/mpif90 \
   --with-cpcomp=${PREREQ_DIR}/bin/mpicc --with-fcomp=ifort --with-ccomp=icc \
   --with-netcdff=${PREREQ_DIR} --with-netcdfc=${PREREQ_DIR} --with-wgrib2=${PREREQ_DIR}
make clean
make FPCOMP=${PREREQ_DIR}/bin/mpif90 CPCOMP=${PREREQ_DIR}/bin/mpicc CCOMP=icc FCOMP=ifort -f Make_utils
make
make install
